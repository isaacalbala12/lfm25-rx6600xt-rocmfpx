...
RESOURCE_SAMPLER=${RESOURCE_SAMPLER:-/home/isaac/Documents/Codex/2026-09-16/new-chat/outputs/lfm2.5-rx6600/scripts/resource_sampler.py}
...

{
  ...
  printf 'concurrencies=%q\ncache_prompt=%q\nprompt_mode=%q\nslot_policy=%q\nignore_eos=%q\narrival_stagger_ms=%q\n' \
    "$CONCURRENCIES" "$CACHE_PROMPT" "$PROMPT_MODE" "$SLOT_POLICY" "$IGNORE_EOS" "$ARRIVAL_STAGGER_MS"
  printf 'fixed_server_args=-ngl\ 99\ -fa\ on\ -np\ 4\ -cb\ %q\ --cache-reuse\ 0\n' "$cache_server_arg"
  printf 'server_args='; printf '%q ' "${SERVER_ARGS[@]}"; printf '\n'
} >"$COMMAND_TXT"

sha256sum "$SERVER" "$MODEL" "$SCRIPT_DIR/bench_client.py" "$RESOURCE_SAMPLER" >"$HASHES_TXT"
...
for concurrency in $CONCURRENCIES; do
  "$PYTHON" "$SCRIPT_DIR/bench_client.py" \
    ...
    --slot-policy "$SLOT_POLICY" \
    --arrival-stagger-ms "$ARRIVAL_STAGGER_MS" "${eos_args[@]}" \
