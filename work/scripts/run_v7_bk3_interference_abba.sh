...
OUTPUT=${OUTPUT:-$ROOT/work/results/v7-interference-selective-bk3-paired3}
PAIRS=${PAIRS:-3}
PORT=${PORT:-18232}
PATCH_APPLIED=0
...
if pgrep -af 'llama-server|vllm|python.*api_server' | grep -v -E 'pgrep|run_v7_bk3_interference' >/dev/null; then
...
cleanup() {
  ...
  if [[ -n "$(git -C "$SOURCE" status --porcelain)" ]]; then
    echo "ROCmFPX source not clean after interference run" >&2
    status=2
  fi
  exit "$status"
}
